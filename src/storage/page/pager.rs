...
pub fn read_page(file: &mut File, page_id: PageId) -> Result<Page, PagerError> {
...
pub fn write_page(file: &mut File, page_id: PageId, page: &Page) -> Result<(), PagerError> {
    let file_len = file.metadata()?.len();
    if file_len % PAGE_SIZE as u64 != 0 {
        return Err(PagerError::InvalidFileSize);
    }
    ...
    if page_id.0 >= file_len / PAGE_SIZE as u64 {
        return Err(PagerError::PageNotAllocated(page_id));
    }
...
pub fn page_count(file: &File) -> Result<u64, PagerError> {
    let file_len = file.metadata()?.len();
    if file_len % PAGE_SIZE as u64 != 0 {
        return Err(PagerError::InvalidFileSize);
    }
...
pub(super) fn page_offset(page_id: PageId) -> Result<u64, PagerError> {
    let offset = page_id
        .0
